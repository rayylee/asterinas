...
///
/// Registered during component initialization so that keyboard interrupts can be
/// handled regardless of whether the full PS/2 device enumeration succeeds. On
/// some platforms the i8042 controller can trigger IRQ1 (e.g. for `SendCtrlAltDel`)
/// but may not respond to device-level PS/2 protocol commands.
...
/// without going through the i8042 driver. This matches the Linux kernel's `BOOT_KBD`
/// approach, which writes directly to the port:
///
