...
#  - OVMF: "on" or "off";
#  - BOOT_METHOD: "elf", "bzimage", "grub-rescue-iso" or "grub-qcow2";
...
# When using direct boot (elf or bzimage), OVMF depends on the boot protocol:
# linux-efi-* protocols require OVMF; other protocols (e.g. multiboot) do not.
if [ "$BOOT_METHOD" = "elf" ] || [ "$BOOT_METHOD" = "bzimage" ]; then
