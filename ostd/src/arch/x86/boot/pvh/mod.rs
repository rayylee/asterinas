...
use crate::{
    boot::{
        BootloaderAcpiArg, EARLY_INFO, EarlyBootInfo,
...
fn finish_memory_regions(
    ...
    if let Some(kernel_cmdline) = kernel_cmdline {
        regions
            .push(MemoryRegion::module(kernel_cmdline.as_bytes()))
            .unwrap();
    }
...
impl HvmStartInfo {
    fn kernel_commandline(&self) -> Option<&'static str> {
        ...
        // SAFETY: The command line is a NUL-terminated string that lives for
        // `'static` by the contract with the VMM.
        let cmdline =
    ...
    fn modules(&self) -> &'static [HvmModlistEntry] {
    ...
    fn initramfs(&self) -> Option<&'static [u8]> {
        ...
        // SAFETY: The module bytes are safe to read and live for `'static` by
        // the contract with the VMM.
        Some(unsafe {
            core::slice::from_raw_parts(
    ...
    fn memory_map(&self) -> Option<&'static [HvmMemmapTableEntry]> {
    ...
    fn memory_regions(
        ...
        finish_memory_regions(regions, initramfs, kernel_cmdline)
    }
    ...
    fn to_early_boot_info(&self) -> EarlyBootInfo {
        ...
        EarlyBootInfo {
            ...
            memory_regions: self.memory_regions(initramfs, kernel_cmdline),
        }
    }
}
