...
pub(super) fn determine_tsc_freq() -> Option<u64> {
    ...
    let pvclock_info = register_pvclock_page()?;
...
/// Allocates the pvclock page, registers it with KVM by writing
/// `MSR_KVM_SYSTEM_TIME_NEW`, and returns a pointer to the page.
fn register_pvclock_page() -> Option<*const PvclockVcpuTimeInfo> {
...
fn read_stable_time_info(pvclock_info: *const PvclockVcpuTimeInfo) -> Option<PvclockTimeSnapshot> {
    ...
    for _ in 0..MAX_RETRIES {
        ...
        let version_before = read_version(pvclock_info);
        ...
        let tsc_to_system_mul = read_tsc_to_system_mul(pvclock_info);
        let tsc_shift = read_tsc_shift(pvclock_info);
        compiler_fence(Ordering::Acquire);

        let version_after = read_version(pvclock_info);
...
fn read_version(pvclock_info: *const PvclockVcpuTimeInfo) -> u32 {
    // SAFETY: `pvclock_info` points to a live KVM pvclock page.
    unsafe { core::ptr::addr_of!((*pvclock_info).version).read_volatile() }
}

fn read_tsc_to_system_mul(pvclock_info: *const PvclockVcpuTimeInfo) -> u32 {
    // SAFETY: `pvclock_info` points to a live KVM pvclock page.
    unsafe { core::ptr::addr_of!((*pvclock_info).tsc_to_system_mul).read_volatile() }
}

fn read_tsc_shift(pvclock_info: *const PvclockVcpuTimeInfo) -> i8 {
    // SAFETY: `pvclock_info` points to a live KVM pvclock page.
